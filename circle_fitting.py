...
def fit_ellipse(points):
    # 1. Find the mean (center of the ellipse)
    center = points.mean(axis=0)

    # 2. Apply PCA to find the orientation and axis lengths
...
def ransac_circle_with_density(points, num_iterations=1000):
...
def ransac_circle(points, num_iterations=1000):
    """Find the circle containing the most points using RANSAC."""
    best_circle = None
    ...
    for _ in range(num_iterations):
        ...
        # Count how many points are inside or on the circle
        distances = np.sqrt((points[:, 0] - xc) ** 2 + (points[:, 1] - yc) ** 2)
        contained_points = np.sum(distances <= radius)

        # Update the best circle if more points are contained
        if contained_points > max_contained_points:
...
def ransac_circle_DISTANCE(points, num_iterations=1000, threshold=1.0):
    """Find the best circle using RANSAC."""
    best_circle = None
    max_inliers = 0
    ...
    for _ in range(num_iterations):
        ...
        # Count inliers
        distances = np.sqrt((points[:, 0] - xc) ** 2 + (points[:, 1] - yc) ** 2)
        inliers = distances[np.abs(distances - radius) < threshold]
