...
class DeepProMP:
    ...
    def __create_em_functions(self, prior_mu_w, prior_Sigma_w):
        ...
        m_uncert_y = tf.matmul(
            self.phi,
            tf.matmul(
                a=tf.tile(tf.expand_dims(self.w_covs, 1), [1, T, 1, 1]),
                b=self.phi,
                transpose_b=True,
            ),
        )
        ...
        m_mah_dist_w = tf.matmul(
            a=m_w_err,
            b=tf.matmul(tf.tile(tf.expand_dims(self.inv_Sig_w, 0), [N, 1, 1]), m_w_err),
            transpose_a=True,
        )
        ...
        self.closed_m_steps = [
            self.mu_w.assign(self.m_step_mu_w),
        ...
        with tf.control_dependencies(update_ops):
            ...
            if len(basis_params) > 0:
                self.opt_basis = tf.train.AdamOptimizer(self.learning_rate).minimize(self.avg_loss, name="train_step", var_list=basis_params)
            else:
                self.opt_basis = None
    ...
    def __init__(
        ...
        if init_Sigma_y is None:
            init_Sigma_y = 1e-2 * np.eye(out_dims)
        if init_mu_w is None:
            init_mu_w = np.zeros((feature_dims, 1))
        if init_Sigma_w is None:
            init_Sigma_w = 1e2 * np.eye(feature_dims)
        self.__diag_sy = True
    ...
    def comp_w_dist(self, sess, inputs, outputs):
        ...
        """
        [w_means, w_covs] = sess.run(
            [self.comp_w_means, self.comp_w_covs],
            feed_dict={self.x: inputs, self.y: outputs, self.is_training: False},
        )
    ...
    def output_dist(self, sess, inputs, w_dist=None):
        ...
        mean_y_pred, cov_y_pred = sess.run(
            [self.mean_y_pred, self.cov_y_pred],
            feed_dict={
                self.x: inputs,
                self.w_means: w_means,
                self.w_covs: w_covs,
    ...
    def __embatch(inputs, outputs, batch_size, sample_size, it, noise_x=None, noise_y=None):
        ...
        for i in range(batch_size):
            ...
            if noise_x is not None:
                x_n += np.random.normal(loc=0, scale=noise_x, size=x_n.shape)
            if noise_y is not None:
                y_n += np.random.normal(loc=0, scale=noise_y, size=y_n.shape)
            x.append(x_n)
            y.append(y_n)
        return np.array(x), np.array(y)

    def __init_basis_fun(
        self,
        sess,
        inputs,
        outputs,
        ...
        with tf.variable_scope("init_phi") as vs:
            ...
            with tf.control_dependencies(update_ops):
                ...
                init_opt_basis = tf.train.AdamOptimizer(self.learning_rate).minimize(init_loss, name="init_phi_train_step", var_list=init_basis_params)
        losses = []
        ...
        for it in range(init_iter):
            ...
            y_n = y
            if noise_x is not None:
                x_n += np.random.normal(loc=0, scale=noise_x, size=x.shape)
            if noise_y is not None:
                y_n += np.random.normal(loc=0, scale=noise_y, size=y.shape)
            loss, void_obj = sess.run(
                [init_loss, init_opt_basis],
                feed_dict={
                    self.x: x_n,
                    self.y: y_n,
    ...
    def fit(
        ...
        """
        N = len(inputs)
        if em_batch_size is None:
            em_batch_size = N
        if basis_batch_size is None:
            basis_batch_size = 1
        if sample_size is None:
            sample_size = min([len(x) for x in inputs])

        ans = {}
        if init_phi_iter > 0:
            ans["init_phi_loss"] = self.__init_basis_fun(
                sess,
                inputs,
                outputs,
                em_batch_size,
        ...
        for it in range(iterations):
            ...
            for j in range(opt_basis_steps):
                ...
                curr_loss, void_obj = sess.run(
                    [self.avg_loss, self.opt_basis],
                    feed_dict={
                        self.x: x[start_ix:end_ix],
                        self.y: y[start_ix:end_ix],
