...
class LinearKF:
    ...
    def _M_step(self, means, covs, Js, As, Bs, Cs, Ds, X, U, fwd_means, **params):
        ...
        P0_N = reduce(
            lambda a, b: a + b,
            map(lambda n: covs[n][0] + np.outer(means[n][0] - mu0, means[n][0] - mu0), xrange(N)),
        )
        if "prior_P0" in params:
            P0_params = params["prior_P0"]
        ...
        if params["debug_LB"]:
            print(
                "Debug. Initial LB=",
                self._EM_lowerbound(means, covs, Js, As, Bs, Cs, Ds, X, U, fwd_means, **params),
            )
        self.mu0 = mu0
        if params["debug_LB"]:
            print("Debug. After updating mu0="), self._EM_lowerbound(means, covs, Js, As, Bs, Cs, Ds, X, U, fwd_means, **params)
        if params["em_print_params"]:
            print("mu0=", self.mu0)
        self.P0 = P0
        if params["debug_LB"]:
        ...
        if params["optimize_A"]:
            ...
            for epoch in range(self.__a_epochs):
                for i in range(len(pairs) / self.__a_bs):
                    ...
                    if params["debug_LB"] and np.random.randint(0, 10000) == 0:
                        # Check gradients
                        my_obj = lambda x: self.stoch_A_err(
                            x,
                            pairs_i,
                            means,
                            covs,
            ...
            if params["debug_LB"]:
                ...
                print(
                    "Debug. After updating A=",
                    self._EM_lowerbound(means, covs, Js, As, Bs, Cs, Ds, X, U, fwd_means, A_params=self.A.params, **params),
                )
            Avalid = False
        ...
        if False:
            ...
            if params["debug_LB"]:
                ...
                print(
                    "Debug. After updating A=",
                    self._EM_lowerbound(means, covs, Js, As, Bs, Cs, Ds, X, U, A_params=self.A.params, **params),
                )
            Avalid = False
        ...
        if not "full_gamma" in params:
            self.Gamma = np.diag(np.diag(self.Gamma))
        if params["debug_LB"]:
            print(
                "Debug. After updating Gamma=",
                self._EM_lowerbound(means, covs, Js, As, Bs, Cs, Ds, X, U, fwd_means, **params),
            )
        if params["em_print_params"]:
        ...
        if not "full_sigma" in params:
            self.Sigma = np.diag(np.diag(self.Sigma))
        if params["debug_LB"]:
            print(
                "Debug. After updating Sigma=",
                self._EM_lowerbound(means, covs, Js, As, Bs, Cs, Ds, X, U, fwd_means, **params),
            )
        if params["em_print_params"]:
    ...
    def EM_train(self, X, U, **params):
        ...
        for i in xrange(max_iter):
            ...
            if "callback_after_E_step" in params:
                params["callback_after_E_step"](self, X, U, means, covs)
