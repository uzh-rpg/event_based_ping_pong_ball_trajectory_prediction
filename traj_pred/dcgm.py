...
class Trajectory:
    ...
    def traj_dist(self, prev_times, prev_obs, pred_times):
        ...
        Xn, Xobs = utils.encode_fixed_dt([prev_times], [self.normalizer.transform(prev_obs)], self.length, self.deltaT)
...
def load_traj_model(path):
    ...
    #     decoder = load_model(os.path.join(path,'decoder.h5'))
    # with CustomObjectScope({'GlorotUniform': glorot_uniform()}):
    #     encoder = load_model(os.path.join(path,'encoder.h5'))

    # decoder = tf.keras.models.load_model(os.path.join(path,'decoder.h5'),custom_objects={'GlorotUniform': glorot_uniform()})
    # encoder = tf.keras.models.load_model("pruned.h5",custom_objects={'GlorotUniform': glorot_uniform()})
    ...
    if os.path.exists(penc_fn):
        ...
        # partial_encoder = tf.keras.models.load_model(penc_fn,custom_objects={'GlorotUniform': glorot_uniform()})

        # with CustomObjectScope({'GlorotUniform': glorot_uniform()}):
        #     partial_encoder = load_model(penc_fn)
...
class TrajDCGM:
    ...
    def __build_graph(self, encoder, partial_encoder, cond_generator, log_sig_y, length, D, z_size):
        self.encoder = encoder
        self.partial_encoder = partial_encoder
